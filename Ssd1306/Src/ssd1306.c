...
// Função auxiliar para enviar bytes via I2C0
status_t SSD1306_WriteBuffer(uint8_t control_byte, uint8_t* buffer, size_t length) {
  lpi2c_master_transfer_t masterXfer = { 0 };
  uint8_t tx_buffer[length];

  /* Monta o pacote: Control Byte + Conteúdo */
  //tx_buffer[0] = control_byte;
  memcpy(tx_buffer, buffer, length);

  memset(&masterXfer, 0, sizeof(masterXfer));
  masterXfer.slaveAddress = SSD1306_I2C_ADDR;
  masterXfer.direction = kLPI2C_Write;
  masterXfer.subaddress = control_byte;
...
{
  SSD1306_WriteBuffer(0x00, &byte, 1);
}

// Send data
void ssd1306_WriteData(uint8_t* buffer, size_t buff_size)
{
  SSD1306_WriteBuffer(0x40, buffer, buff_size);
}
...
void ssd1306_Init(void)
{
  ...
  SDK_DelayAtLeastUs(100000, LPI2C0_CLOCK_FREQ); // Delay for 100ms (assuming a 48 MHz clock)
...
void ssd1306_UpdateScreen(void)
{
  ...
  for (uint8_t i = 0; i < (SSD1306_HEIGHT / 8); i++)
  {
    ...
    ssd1306_WriteCommand(2 & 0x0F);//0x00 + SSD1306_X_OFFSET_LOWER);
    ssd1306_WriteCommand(0x10 | (2 >> 4));//0x10 + SSD1306_X_OFFSET_UPPER);
  ...
  while (pY < Y + H)
  {
    ...
    while (pX < X + W)
    {
      ...
      if (tmpCh)
      {
        ...
        {
          if (tmpCh & 0x01)
            ssd1306_DrawPixel(pX, pY + bL, color);
          tmpCh >>= 1;
          if (tmpCh)
          {
            bL++;
...
void ssd1306_BarGrafic(uint8_t x, uint8_t y, uint8_t w)
{
  if (w > SSD1306_WIDTH)
    w = 0;
  // ssd1306_Line(1, 28, SSD1306_WIDTH - 1, 28, White);
  ssd1306_Line(x, y, w, y, White);
  ssd1306_Line(x, y + 1, w, y + 1, White);
  ssd1306_Line(x, y + 2, w, y + 2, White);
  ssd1306_Line(x, y + 3, w, y + 3, White);
  ssd1306_Line(x, y + 4, w, y + 4, White);
  ssd1306_UpdateScreen();
}

void ssd1306_print_sn(uint8_t ID, uint8_t tipo, uint8_t* SN)
{
  char texto[50];
  ssd1306_Clear();
  ...
  case 0xAD:
    ...
    break;
  default:
    sprintf(texto, "IGBR:%.2d:", ID);
    break;
  }

  ssd1306_WriteString(texto, Font_7x10, White);
  ssd1306_SetCursor(58, 8);
  memset(texto, 0, sizeof(texto));
  for (uint8_t count = 0; count < 8; count++)
  {
    sprintf(texto + count, "%d", SN[count]);
  }
  ssd1306_WriteString(texto, Font_7x10, White);
  ssd1306_UpdateScreen();
}
