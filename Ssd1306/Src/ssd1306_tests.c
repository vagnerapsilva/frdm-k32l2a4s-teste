...
extern volatile uint32_t g_msCount;
/*!
...
uint32_t HAL_GetTick(void)
{
  ...
  return currMsCount + ((float)loadTick - (float)currTick) / (float)loadTick;
}
...
{
  ssd1306_Fill(White);
  ssd1306_DrawBitmap_inv(0, 0, garfield_128x64, 128, 64, Black);
  ssd1306_UpdateScreen();
  HAL_Delay(3000);
  ssd1306_Fill(Black);
  ssd1306_DrawBitmap_inv(32, 0, github_logo_64x64, 64, 64, White);
  ssd1306_UpdateScreen();
  HAL_Delay(3000);
  ssd1306_Fill(White);
  ssd1306_DrawBitmap_inv(32, 0, github_logo_64x64, 64, 64, Black);
  ssd1306_UpdateScreen();
}
